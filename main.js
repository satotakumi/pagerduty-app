...
function createWindow() {
  ...
  setInterval(() => {
    pdApi
      .fetchTriggeredIncidents(
        userId,
        dayjs().subtract(600, 'minute').toISOString()
      )
      .then((incidents) => {
        ...
        if (incidents.find((incident) => incident['status'] === 'triggered')) {
          ...
          tray.setImage(
            generateNativeImageIcon(
              nativeTheme.shouldUseDarkColors ? 'white' : 'gray'
            )
          )
        }
        ...
        incidents.forEach((incident) => {
          ...
          contextMenuTemplate.push({
            ...
            icon:
              incident['status'] === 'triggered'
                ? generateNativeImageIcon('red')
                : generateNativeImageIcon('yellow'),
            click: () => {
          ...
          // 通知済み、もしくはAck済みはスキップ
          if (
            notified_incident_ids.includes(incident['incident_number']) ||
...
function generateNativeImageIcon(color = null) {
  let nativeImageFile = nativeImage.createFromPath(
    __dirname + '/assets/images/icon.png'
  )
  if (color === 'gray') {
    nativeImageFile = nativeImage.createFromPath(
      __dirname + '/assets/images/icon_gray.png'
    )
  } else if (color === 'white') {
    nativeImageFile = nativeImage.createFromPath(
      __dirname + '/assets/images/icon_white.png'
    )
  } else if (color === 'yellow') {
    nativeImageFile = nativeImage.createFromPath(
      __dirname + '/assets/images/icon_yellow.png'
    )
  } else if (color === 'red') {
    nativeImageFile = nativeImage.createFromPath(
      __dirname + '/assets/images/icon_red.png'
    )
  }
